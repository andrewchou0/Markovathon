...
export default function ResponsePanel({ result, stage, regenerating, elapsed, tab, onTab, error, notice, onCopy, onRegenerate }) {
  ...
  return (
    <section className="result-panel panel" id="response" aria-busy={busy || regenerating} aria-labelledby="response-title">
      ...
      {!result && !busy && <div className="empty-result"><span className="empty-icon"><Icon name="network" size={27} /></span><h3>Understand the impact first.</h3><p>Run a demo to see affected suppliers, a risk assessment, and a response draft.</p></div>}
      {busy && <div className="loading-result" role="status"><div className="loading-title"><span className="spinner" /><strong>{STAGES[stage]}…</strong><span>{elapsed}s</span></div><p>{stage === 1 ? 'Reading the disruption and its affected location.' : stage === 2 ? 'Revealing direct impact, then downstream exposure.' : 'Preparing the sample assessment and email draft.'}</p><div className="skeleton" aria-hidden="true"><i /><i /><i /></div><small>Demo playback uses a sample response.</small></div>}
      {result && <>
        ...
        <div role="tabpanel" id="draft-panel" aria-labelledby="draft-tab" hidden={tab !== 'draft'} className="draft-panel">
          ...
          <small className="draft-disclaimer">Demo mode reloads the sample draft.</small>
        </div>
